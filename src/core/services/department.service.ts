...
export const getDepartmentById = async (id: number, organization_id: number): Promise<Department | null> => {
  if (!Number.isInteger(id)) {
    throw new Error(`Invalid department ID: ${id}`);
  }
  return Department.findOne({ where: { department_id: id, organization_id } });
};
...
export const getDepartmentsByParams = async (params: DepartmentQueryDTO): Promise<Department[]> => {
  ...
  (filters as any).organization_id = (params as any).organization_id;
  return Department.findAll({ where: filters });
};

export const deleteDepartment = async (id: number, organization_id: number): Promise<boolean> => {
  const department = await Department.findOne({ where: { department_id: id, organization_id } });
  if (!department) return false;
  await department.destroy();
  return true;
};

export const updateDepartment = async (id: number, data: UpdateDepartmentDTO, organization_id: number): Promise<Department | null> => {
  const department = await Department.findOne({ where: { department_id: id, organization_id } });
