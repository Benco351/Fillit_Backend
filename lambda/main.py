...
def lambda_handler(event, context):
    # Only set EMPLOYEE_ID and ADMIN_MODE from context
    config.EMPLOYEE_ID = context.get('EMPLOYEE_ID', config.EMPLOYEE_ID)
    config.ADMIN_MODE = context.get('ADMIN_MODE', config.ADMIN_MODE)
    ...
    input_messages = [
        {
            "role": "developer",
            "content": f"You are an AI assistant on a shift management platform. You are limited to {config.MAX_TOKENS} tokens in your response."
        },
