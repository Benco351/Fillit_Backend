...
def get_departments(config: Config) -> List[Dict[str, Any]]:
    """Fetch all departments with their names and addresses (if available)."""
    try:
        resp = requests.get(f"{config.base_url}/api/departments", headers=_auth_header(config))
        resp.raise_for_status()
        return resp.json().get("data", [])
    except requests.RequestException as exc:
        return [{"status": "error", "message": str(exc)}]
